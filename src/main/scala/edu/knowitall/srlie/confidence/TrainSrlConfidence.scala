...
object TrainSrlConfidence {
  ...
  def run(settings: Settings) = {
    ...
    val gold = input.flatMap {
      case (annotation, extraction, sentence) =>
    ...
    if (settings.evaluate) {
      ...
      val annotated = for {
        ...
        _ = require(
          (test.map(_.text).toSet intersect training.map(_.text).toSet) == Set.empty,
      ...
      sorted.filter(_._2 == false) foreach {
        case (conf, annotation, ex) =>
